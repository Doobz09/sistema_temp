...
#define GPIO_OUTPUT_PIN LED4    // LED en GPIO 2 (normalmente el LED integrado)
#define GPIO_OUTPUT_PIN_2 LED3    // LED en GPIO 2 (normalmente el LED integrado)
...
void IRAM_ATTR gpio_isr_handler(void* arg) {
    ...
    if(pin == GPIO_SENSOR_1){
        ...
        if (adc_task_handle == NULL) {
            ...
            if(gpio_get_level(GPIO_SENSOR_2)==0){
                bandera_control=212;
            }
            else{
                bandera_control=11;
    ...
    if(pin == GPIO_SENSOR_2){
        ...
        if (adc_2_task_handle == NULL) {
            ...
            if(gpio_get_level(GPIO_SENSOR_1)==0){
                bandera_control=212;
            }
            else{
                bandera_control=12;
    ...
    int led_state = gpio_get_level(GPIO_OUTPUT_PIN);
    gpio_set_level(GPIO_OUTPUT_PIN,1);  // Cambiar estado del LED
